...
2. Run the complete backend quality gate with `mvn verify`.
3. Run `pnpm install --frozen-lockfile` and `pnpm build` in `frontend/`.
4. When container files change, run `docker compose -f compose.prod.yml build`.
5. Keep changes focused and explain user-visible behavior in the pull request.
